...
def train_process(global_agent, global_memory, vae, rnn, update_term, pid, state_dims, hidden_dims, lr, device=None, seed=0):
    print('train_process starting', vae.training)
    set_seed(seed)

    env = gym.make('CarRacing-v0')
    #env.monitor.start('/tm/cartpole', force=True)
    env.verbose = 0
#     time_limit = 1000
#     #env.render()

    #agent = A3C(input_dims=state_dims, hidden_dims=hidden_dims, lr=lr, memory_model=ltm).to(device)
    ...
    for ep in range(max_ep):
        ...
        #print('sanity check', '\n', vae.encoder(obs2tensor(obs)[0])[0].shape)
        ...
        with torch.no_grad():
            ...
            next_latent_mu, _ = vae.encoder(next_obs)#vae.encoder(next_obs)
        while True:
            print(i)
            # #env.render()
            obs = next_obs
            ...
            if done:
                print('final score {}'.format(reward))
...
def test_process(global_agent, global_memory, vae, rnn, update_term, pid, state_dims, hidden_dims, lr, device=None, seed=0):
    print('test_process starting')
    env = gym.make('CarRacing-v0')
    #env.monitor.start('/tm/cartpole', force=True)
    set_seed(seed, env=env)
    env.verbose = 0
    ##env.render()
    ltm = episodicRecall(hidden_size=state_dims)
    agent = A3C(input_dims=state_dims, hidden_dims=hidden_dims, lr=lr, memory_model=ltm).to(device)
    ...
    for ep in range(test_ep):
        ltm.load_state_dict(global_memory.state_dict())
        agent.load_state_dict(global_agent.state_dict())
        env.reset()
        score = 0.
        i = 0
...
state = torch.load(vae_path, map_location={'cuda:0': str(device)})
...
state = torch.load(rnn_path, map_location={'cuda:0': str(device)})
...
global_memory = episodicRecall(hidden_size=state_dims)
global_agent = A3C(input_dims=state_dims, hidden_dims=hidden_dims, lr=lr, memory_model=global_memory).to(device)
...
#for pid in range(n_processes+1):
    #if pid == n_processes:
        #print('test test')
        #p = mp.Process(target=test_process, args=(global_agent, global_memory, vae, rnn, update_term, pid, state_dims, hidden_dims, lr,))
    #else:
        #print('train train train')
p = mp.Process(target=train_process, args=(global_agent, global_memory, vae, rnn, update_term, 2, state_dims, hidden_dims, lr,))
p.start()
processes.append(p)
